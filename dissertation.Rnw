...
\parencite{hakamata_attention_2010} and more recently ABM has been
shown to reduce depressive symptoms \parencite{yang_attention_2015}. Critics
of CBM argue that the effect sizes are too small to be clinically
relevant \parencite{cristea_efficacy_2015}. However, advocates argue
that this critique is based on meta-analyses which don't differentiate
ABM from other forms of CBM, don't focus on a single psychological
dysfunction (anxiety), or don't analyse studies which demonstrate both
...
\item Trait worry, rumination over the previous month, and anxiety and
depression over the previous 2 weeks will reduce after ABM training.
...
however all differences were non-significant (${P}$s < .05), as was the
meta-analysis (${p}$ = .187).
...
\subsection{ROF/WTDD (200)}

This study aimed to replicate previous findings
\parencite{yang_attention_2015} that ABM training can reduce attentional
bias towards negative words and that this change is associated with
concurrent reductions in trait rumination and depressive symptoms. In
so doing, it also sought test the hypothesis that rumination occurs as
a result of an impaired ability to disengage from negative self-referent
information \parencite{koster_understanding_2011}.

Attentional bias towards ideographic words associated with a
current concern will reduce after ABM training.

Trait worry, rumination over the previous month, and anxiety and
depression over the previous 2 weeks will reduce after ABM training.

After ABM training, state rumination, negative affect and depressive
symptoms will reduce, and state positive affect will increase.
...
Pre-post. Point out that these are trait measures so relatively coarse.
...
(PHQ-9) and anxiety (GAD-7) in the current study.
...
Did it work for some but not others? e.g. see trend reversal in NA for p8

Were most Ps high worry but the dot-probe exposure configured for high rumination (1250ms)?

\subsection{CORE (1000-1200}
...
More specific mechanistic hypotheses e.g. "rumination was related
...
\subsubsection{CBM vs. other treatments}

CBM effect sizes are too small to be clinically relevant \parencite{cristea_efficacy_2015}
...
Whilst the design lacks a control condition, it does provide some
controls against alternative explanations of effects. Replication
across participants controls for individual regression to
the mean\todo{Nick questioned this!}, variable phase A duration controls for baseline
assessment effects, and randomised transition to phase B
controls for extraneous events correlated with phase transition

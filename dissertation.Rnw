...
Randomization is a nonparametric approach which allows inferential
...
which phase A transitions to phase B which is randomized. Under
...
from A to B occurs. Randomization tests hold the observation scores
...
\texttt{A-B}\rvert\)).  In addition to the single-case randomization
...
${p}$=.05 level, even though the randomization tests could not detect

...
We chose a case series design, as this was a small-scale,
proof-of-principle study with insufficient funds for recruiting sufficient
participants required for an adequately powered group study.  Details of
the design can be found in the \nameref{analyses} section.
...
and, to include high trait ruminatiors or trait worriers,
 a Ruminative Responses Scale (RRS)
\parencite{treynor_rumination_2003} score ${\geq}$ 53 or a Penn
...
participants in the top 33\% of these measures, based on a sample of 210
University of Exeter undergraduate students measured on RRS (${M}$ = 46.9,
${SD}$ = 12.7) and PSWQ (${M}$ = 48.2, ${SD}$ = 14.9)\todo{cite?}.
...
\subsubsection{Primary Measures}

Participants were measured on primary outcomes approximately every
24 hours (except where sessions were postponed). Attentional bias was
measured using a computer-based dot-probe task with word-pair stimuli
\parencite{macleod_attentional_1986}.  State rumination was measured using
the Goal Rumination Scale (GRS) \parencite{schultheiss_role_2008}, the
best scale we were aware of that would measure state rumination towards
a current concern, in line with our hypotheses\todo{H1?}. Reliability of
the GRS is good (Cronbach's $\alpha$ = .87), based on a single sample
of $N$ = 101. Positive and negative affect were measured using the
10 item, short form Positive and Negative Affect Schedule (PANAS-SF)
\parencite{mackinnon_short_1999}, which has Cronbach's $\alpha$ of .78
for positive affect (PA) and .87 for negative affect (NA).  Because we
were specifically interested in changes in depression and anxiety, we
added 4 items to the PANAS-SF.  Items 'sad' and 'depressed' were chosen
as a state measure of depression, the rationale being that they would
correlate negatively with PA and positively with NA.  Items 'anxious'
and 'worried' were added as a state measure of worry \todo{I need to have
committed to anxiety or worry by this point!} as they were expected to
correlate positively with NA. \todo{need a reference to show lack of
items at low end of PA and that low PA/high NA == depression.} Questions
on the GRS and PANAS-SF were asked in relation to the last 24 hours.

\subsubsection{Secondary Measures}

Secondary measures were taken before the first and after the
last dot-probe/ABM sessions.  Trait rumination was measured
using the Ruminative Responses Scale (RRS), which is reliable
and valid, having an expected Cronbach's $\alpha$ of .85
\parencite{treynor_rumination_2003}. Trait anxiety was measured
using the 7 item Generalised Anxiety Disorder (GAD-7) scale, an
efficient, valid (Cronbach's $\alpha$ > .85) tool for assessing GAD
severity in clinical research, with good test-retest reliability
\parencite{spitzerrl_brief_2006}. Depression was measured using
the PHQ-9, a brief, reliable (Cronbach's $\alpha$ > .85) and valid
measure of depression severity \parencite{kroenke_phq9_2001}. Trait
worry was measured using the Penn State Worry Questionnaire (PSWQ),
an an internally consistent measure with good test-retest reliability
\parencite{meyer_development_1990}.  Questions on the RRS and GAD-7
were asked in relation to the previous 4 weeks of participants' lives,
to correspond approximately with their time within the study\todo{see
appendix}.

\subsection{Materials}

Two sets of visual word stimuli were used.  The 18 words chosen by
the participant in relation to their incomplete goal will be referred
to as the 'G set'. The 'P set' will be common to all participants and
comprise 167 words selected from a pool of abstract, sad words following
the procedure in \textcite{macleod_selective_2002}. Each word from the G
and P sets will be paired with a corresponding neutral word, matched for
letter length, frequency of usage, valence, arousal, imagery, familiarity
and relevance to sadness \parencite{yang_attention_2014}. The total
of 185 word pairs allows for 1 practice and 36 experimental dot-probe
test sessions even when the 18 G set words fully overlap with P set
words.  Dot-probe and ABM tasks will be developed using OpenSesame
\parencite{mathot_opensesame_2011} and measurement instruments
administered online using Lime Survey \parencite{limesurvey}.

\todo{SAWL - How are affective word ratings related to lexicosemantic
properties? (Citron, Weekes, \& Ferstl, 2014)}
...
\subsubsection{MRC}
{
{}``These programs permit the construction of word sets for psycholinguistic experiments that control for the attributes
specified in the dictionary.'' (Wilson, 1988)}

{
Web UI to dictionary software: http://websites.psychology.uwa.edu.au/school/MRCDatabase/uwa\_mrc.htm}

{
Word ratings \ (9-point scales) (Yang, Ding, Dai, Peng, \& Zhang, 2014)}
...
{
{}``The 247 candidate words were rated by an independent sample of 53 college students (25 female, mean age: 19.8 [SD =
1.00], BDI-II score: 9.04 [SD = 7.74], range: 0-42, including 8 dysphoric individuals)''}

{
{}``Each pair selected for use contained a sad member and a neutral member, in which the mean ratings of sad and neutral
member differed in emotional valence and relevance to sadness (ps {\textless} 0.001)%
%t{}-tests
%Paul Sharpe
%March 30, 2015 4:09 PM
, but matched in terms of arousal, imagery, familiarity and the number of strokes comprising the words (ps
{\textgreater} 0.05)%
%More t{}-tests
%Paul Sharpe
%March 30, 2015 4:24 PM
 (see Supplemental Table 1). In addition, the difference of the mean valence between the two members of any selected
pair was at least 1.45 (Mean: 2.64 [SD = 0.56]).''}
...
Affective Word List \parencite{citron_how_2014}. Starting with the 66
...
{
N-word stimulus pairs (see Appendix 1) were created using the Sussex Affective Word List (Citron et al., 2014). Starting
with the 66 words rated most negative and and 66 rated most neutral, pairs were created by matching words of equivalent
length (number of characters). \ Additional words were drawn from the pool to ensure that all pairs were of equal
length. \ A small number of pairings which the researcher considered to be semantically related where adjusted by
swapping neutral words in the pair. \ Independent samples t-tests showed significant differences for mean ratings of
valence and arousal (\textit{ps} {\textless} .001). \ Pairs were matched for frequency, imageability, familiarity,
number of characters and number of syllables (all \ \textit{ps} {\textgreater} .16).%
%Add t{}-tests in appendix
%Paul Sharpe
%May 31, 2015 4:25 PM
 \ In addition, the mean valence between members of any pair was at least 1.44 \ (Mean: 2.15 [\textit{SD} = 0.37]).
\ Pairs were ranked by mean valence difference. \ \textcolor{black}{The 6 pairs with the lowest difference were
assigned to the ``spare'' block. \ The remaining 60 items were divided into 6 groups, and the experimental blocks were
created by taking the next highest ranked pair from each group.}}
...
I-words were paired with neutrally valenced words using a database populated with frequency, imageability, familiarity,
word length and number of syllables from letter from the MRC psycholinguistic database (Wilson, 1988), supplemented
with 1030 word valence means from the Affective Norms for English Words (ANEW) dataset (Bradley \& Lang, 1999). \ Two
different matching approaches were used, depending on whether the I-word was present in, or absent from, the MRC
database. \ If present, a pair was selected from a pool of words with a frequency {\textpm}15\% that of the I-word, an
ANEW valence mean (range 1-9) between 4.00 and 6.00 and an equal number of characters. \ After selecting a pool of
matched words on the preceding criteria, a best match was found by inspection of candidate words based on number of
syllables, imageability and familiarity. \ Some I-words, such as long words or high frequency words, could not be
matched using these criteria due to lack of available data. \ In these cases, a word with the most neutral valence was
chosen that also minimised differences in length

\todo{And frequency?}

 between the word pair. \ I-words absent from the MRC database were paired with words selected from a pool of 56 words
having both an ANEW valence rating and a frequency of 1 (on the assumption that words missing from the database would
most likely be of low frequency). \ A word was selected from this pool having an ANEW valence mean between 4.00 and
6.00 and an equal number of characters to the I-word. \ If an I-word or its pair were present in the N-words, the
N-word pair was substituted with the pair from the spare N-words block with the largest valence difference. \ %

\todo{Where an I{}-word is already and N{}-word, should its pair be selected using same procedure as for other I{}-words or should the N{}-word pair be used?}

Descriptive statistics for each participant's I-word pairs are shown in Appendix 2.

\subsection{Attentional Bias Modification (ABM)}

\subsubsection{Dot-probe task}

The dot-probe task was based on \textcite{yang_attention_2014}, and
...
possible. \ Each dot-probe block will take approximately 5 minutes.
...
Inaccurate trials or those with response times exceeding 3 standard
deviations beyond the mean were excluded from analyses. Attentional
bias scores were calculated from the remaining response times using
the equation \begin{displaymath}score = [(NuPl + NlPu) = (NuPu + NlPl)]/2 \ \ \ \
(1) \end{displaymath} where ${N}$ = Negative word, ${P}$ = Probe, ${u}$ = upper, ${t}$ = lower
\parencite{bradley_attentional_1997}.

More positive scores reflect an attentional bias towards (i.e., vigilance
for) negative words relative to neutral words, whereas more negative
scores indicate avoidance \parencite{bradley_attentional_1997}.
...
Psychology department  and take about 45
minutes to complete.  During this session you will be asked to generate
some words associated with an unresolved problem or concern that you
currently have. You will then complete some questionnaires about your
thoughts and mood, and a computer task which involves reading pairs of
words and then detecting the location of dots on the screen.
...
part in the study. A self-relevant, unresolved goal will be
...
\todo{stimulus allocation table needs a major fixup}
\captionof{table}[]{}

Example stimulus allocation for a randomisation pattern which transitions
from phase A (dot-probe test) to phase B (ABM training) at session 12.
...
~
 &
\centering 96/48 = 2 &
\centering 96/52\footnote{Where necessary, stimuli will be randomly excluded until exposures are exactly divisible by total stimuli.}\footnote{\label{stimuli}Where total stimuli * 2 {\textgreater} exposures, stimuli will be randomly excluded until total stimuli * 2 = exposures.} &
\centering ... &
\centering\arraybslash 96/144\textsuperscript{\ref{stimuli}}\\\hhline{~---------}
 &
Exposures/G pairs &
~
 &
...
\section{\label{analyses}Planned Analyses}

\todo{past tense}

 More specifically, this was
an AB phase nonconcurrent multiple baseline replication design
across participants\todo{sure?!!} \parencite{bulte_when_2012}
\parencite{onghena_customization_2005}

Comparisons were made between each participant's baseline (phase A)
scores and their post-training (phase B) scores and a meta-analysis
...
Randomisation procedures were planned to allow nonparametric inferential
statistics to be calculated for each participant \parencite{bulte_r_2008}.
With a minimum of 8 ABM training sessions and 35 measurement times (MTs)
in total, there were 21 possible transition points between phases A and B,
which allowed significance testing at the .05 level for each participant
who completed all sessions \parencite{onghena_customization_2005}.
...
Paired samples t-tests were used to compare pre and post ABM training
measures of trait rumination (RRS), trait anxiety (PSWQ) and depression
(PHQ-9).  However, these analyses were mostly descriptive as trait
measures the RRS and PSWQ are relatively coarse, and all comparisons
were under-powered due to the small sample size \todo{"reverse power
analysis"}.

The main analyses are the daily measures ...Visual analyses of each
...
... but now we can look at things individually, visually and inferentially ...
...
\todo{Describe any patterns in each table, see \parencite{watkins_rumination-focused_2007} including MA}
...
\todo{ABM vs. IBM?}
Differences in stimulus type e.g. words vs. faces
...
\tablelasttail{}
{\footnotesize
\begin{supertabular}{m{3cm}m{14.511001cm}}
\hline
{ Pair number} &
\centering\arraybslash{ N-word pairs (Negative-Neutral)\footnotemark{}%
%Present these organised by their blocks (see 66\_nwords.csv)
%Paul Sharpe
%July 3, 2015 3:07 PM
}\\\hline
\centering{ 1} &
\centering\arraybslash{ Conflict-Opposite}\\
\centering{ 2} &
\centering\arraybslash{ Misfortune-Collective}\\
\centering{ 3} &
\centering\arraybslash{ Cry-Boy}\\
\centering{ 4} &
\centering\arraybslash{ Murder-Tricky}\\
\centering{ 5} &
\centering\arraybslash{ Afraid-Leader}\\
\centering{ 6} &
\centering\arraybslash{ Betrayed-Matching}\\
\centering{ 7} &
\centering\arraybslash{ Defeated-Scissors}\\
\centering{ 8} &
\centering\arraybslash{ Devil-Weave}\\
\centering{ 9} &
\centering\arraybslash{ Victim-Desert}\\
\centering{ 10} &
\centering\arraybslash{ Lonely-Cellar}\\
\centering{ 11} &
\centering\arraybslash{ Argument-Remember}\\
\centering{ 12} &
\centering\arraybslash{ Agony-Abbey}\\
\centering{ 13} &
\centering\arraybslash{ Suspicious-Discussion}\\
\centering{ 14} &
\centering\arraybslash{ Disappointed-Concentrated}\\
\centering{ 15} &
\centering\arraybslash{ Worthless-Situation}\\
\centering{ 16} &
\centering\arraybslash{ Anxious-Release}\\
\centering{ 17} &
\centering\arraybslash{ Loss-Bold}\\
\centering{ 18} &
\centering\arraybslash{ Failure-Joining}\\
\centering{ 19} &
\centering\arraybslash{ Excluded-Ordinary}\\
\centering{ 20} &
\centering\arraybslash{ Terrified-Apathetic}\\
\centering{ 21} &
\centering\arraybslash{ Damage-Memory}\\
\centering{ 22} &
\centering\arraybslash{ Crime-Guest}\\
\centering{ 23} &
\centering\arraybslash{ Furious-Meeting}\\
\centering{ 24} &
\centering\arraybslash{ Divorce-Teacher}\\
\centering{ 25} &
\centering\arraybslash{ Punished-Follower}\\
\centering{ 26} &
\centering\arraybslash{ Brag-Tree}\\
\centering{ 27} &
\centering\arraybslash{ Weapon-Valley}\\
\centering{ 28} &
\centering\arraybslash{ Loser-Faith}\\
\centering{ 29} &
\centering\arraybslash{ Nazi-Tell}\\
\centering{ 30} &
\centering\arraybslash{ Depressed-Abundance}\\
\centering{ 31} &
\centering\arraybslash{ Bad-Job}\\
\centering{ 32} &
\centering\arraybslash{ Cowardly-Sympathy}\\
\centering{ 33} &
\centering\arraybslash{ Hateful-Defence}\\
\centering{ 34} &
\centering\arraybslash{ Attack-Return}\\
\centering{ 35} &
\centering\arraybslash{ Hurt-Book}\\
\centering{ 36} &
\centering\arraybslash{ Poverty-Morning}\\
\centering{ 37} &
\centering\arraybslash{ Abandon-Witness}\\
\centering{ 38} &
\centering\arraybslash{ Distressed-Astonished}\\
\centering{ 39} &
\centering\arraybslash{ Harm-Milk}\\
\centering{ 40} &
\centering\arraybslash{ Ashamed-Opinion}\\
\centering{ 41} &
\centering\arraybslash{ Angry-Build}\\
\centering{ 42} &
\centering\arraybslash{ Hell-Poet}\\
\centering{ 43} &
\centering\arraybslash{ Illness-Herring}\\
\centering{ 44} &
\centering\arraybslash{ Lie-Vow}\\
\centering{ 45} &
\centering\arraybslash{ Threat-Source}\\
\centering{ 46} &
\centering\arraybslash{ Accused-Curious}\\
\centering{ 47} &
\centering\arraybslash{ Prison-Member}\\
\centering{ 48} &
\centering\arraybslash{ Bomb-Lake}\\
\centering{ 49} &
\centering\arraybslash{ Frightened-Intonation}\\
\centering{ 50} &
\centering\arraybslash{ Destroy-Ceiling}\\
\centering{ 51} &
\centering\arraybslash{ Useless-Whisper}\\
\centering{ 52} &
\centering\arraybslash{ Nightmare-Housewife}\\
\centering{ 53} &
\centering\arraybslash{ Crisis-Forest}\\
\centering{ 54} &
\centering\arraybslash{ Evil-Rise}\\
\centering{ 55} &
\centering\arraybslash{ Stinking-Receiver}\\
\centering{ 56} &
\centering\arraybslash{ Enemy-Organ}\\
\centering{ 57} &
\centering\arraybslash{ Collapse-Activity}\\
\centering{ 58} &
\centering\arraybslash{ Stressed-Language}\\
\centering{ 59} &
\centering\arraybslash{ Sad-Lip}\\
\centering{ 60} &
\centering\arraybslash{ Corpse-Salary}\\
\centering{\itshape 61} &
\centering\arraybslash{\itshape Despair-Parting}\\
\centering{\itshape 62} &
\centering\arraybslash{\itshape Wound-Attic}\\
\centering{\itshape 63} &
\centering\arraybslash{\itshape Battle-Choose}\\
\centering{\itshape 64} &
\centering\arraybslash{\itshape Punch-Change}\\
\centering{\itshape 65} &
\centering\arraybslash{\itshape Chaos-Pupil}\\
\centering{\itshape 66} &
\centering\arraybslash{\itshape Burden-Accent}\\\hline
~
 &
~
\\
...
\end{flushleft}
\footnotetext{N-word pairs 61-66 in \textit{italic} make up the ``spare'' block which was drawn on when I-words were
also N-words. }
...
\todo{Nick: As you are going to the trouble of presenting this info and you have few participants, it probably makes sense to present the means and SDs for the group of I words AND \ the group of paired words separately for each participant. You could use some kind of subscript to indicate which database the relevant statistic was taken from. T{}-test doesn{}'t make much sense here so descriptives alone are fine.}
\begin{flushleft}
\tablefirsthead{}
\tablehead{}
\tabletail{}

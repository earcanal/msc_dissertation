...
Questions on the RRS and GAD-7 were asked in relation to the previous
4 weeks of participants' lives, to correspond approximately with the
month before the study (pre) and time within the study (post).\todo{see
appendix}

\subsubsection{Ruminative Responses Scale}

Trait rumination was measured using the Ruminative Responses Scale (RRS),
which is reliable and valid, having an expected Cronbach's $\alpha$
of .85 \parencite{treynor_rumination_2003}.

\subsubsection{Generalised Anxiety Disorder}

Trait anxiety was measured using the 7 item Generalised Anxiety Disorder
(GAD-7) scale, an efficient, valid (Cronbach's $\alpha$ > .85) tool
for assessing GAD severity in clinical research, with good test-retest
reliability \parencite{spitzerrl_brief_2006}.

\subsubsection{Patient Health Questionnaire}

Depression was measured using the PHQ-9, a brief, reliable
(Cronbach's $\alpha$ > .85) and valid measure of depression severity
\parencite{kroenke_phq9_2001}.

\subsubsection{Penn State Worry Questionnaire}

Trait worry was measured using the Penn State Worry Questionnaire (PSWQ),
an an internally consistent measure with good test-retest reliability
\parencite{meyer_development_1990}.


\subsection{Daily Measures}

Participants were measured approximately every 24 hours (except where
sessions were postponed). Questions on the GRS and PANAS-SF were asked
in relation to the last 24 hours.

\subsubsection{Attentional bias}

Attentional bias was measured using a computer-based dot-probe task with
word-pair stimuli \parencite{macleod_attentional_1986}.

\subsubsection{Goal Rumination Scale}

State rumination was measured using
...
\subsubsection{Positive and Negative Affect Schedule}
...
items at low end of PA and that low PA/high NA == depression.}

...
\todo{aspell}
\todo{BODY: 8000 words max excluding toc, footnotes, tables, figures, references}
\todo{ABSTRACT use keywords (suggestion < 13)}
\todo{check draft against Monsell and Pinker}
\todo{Word count of main text on front page (latexcount.pl)}

\section{Introduction}

\paragraph{Depression and anxiety would benefit from more effecitve, cheaper interventions grounded in evidence of the underlying cognitive mechanisms associated with these disorders.}
...
household survey \parencite{mcmanus_adult_2009}. Whilst this reflects
...
depression will be the 2nd most common cause of disability-adjusted life
years (DALY). Based 2007 prevalance rates, \textcite{mccrone_paying_2008}
estimate that the combined cost of depression and anxiety services in
England in 2026 will be \pounds5 billion, with lost employment bringing
the total cost to \pounds26.4 billion.

"recommendations include ... the expansion of evidence-based interventions in primary care settings for people
with depression and anxiety disorders" \parencite{mccrone_paying_2008}

"Primary care trusts should commission more evidence-based interventions in primary
care settings for people with depression and anxiety disorders, and providers should
aim to treat more people who have these disorders but currently do not receive
treatment. The reduction in lost employment costs should outweigh the cost of
increased service provision. Psychological therapies are evidence-based and popular,
and access to such services should be extended. However, depending on their success
rate and the growth in real earnings among people who return to work as a result, they
may be a costly option. As such, more focus should be placed on the most cost-
effective ways of delivering such care (for example, through computer-delivered
cognitive behavioural therapy)." \parencite{mccrone_paying_2008}

Treatment efficacy

" a review of meta-analyses identified CBT response rates of 50\% or less
for two of the three DSM-IV anxiety disorders for which meta-analyses
have been conducted (Hofmann, Asnaani, Vonk, Sawyer, \& Fang, 2012). Thus,
even within this gold standard, first-line treatment, there is substantial
room for improvement." \parencite{schneider_state_2015}

"pharmacological and psychological treatments together can reduce
the disease burden of depression by only about 33\%" \textcite{cuijpers_psychotherapies_2015}
 citing \textcite{andrews_utilising_2004}

What to do?!

"I-CBT is a viable treatment option for adults with
depression and some anxiety disorders who request this treatment
modality. Important questions remain before broad implementation can be
supported." \parencite{arnberg_internetdelivered_2014}

"Innovative applications such as mobile apps [71] and real-time monitoring
of mood and behavior (ecological momentary assessment) [72] may have the
promise to realize that, but future research will show whether this will
succeed or not." \textcite{cuijpers_psychotherapies_2015}

Understanding underlying cognitive mechanisms

Anti-depressant medication (ADM) and psychological therapies for
depression such as CBT and MBCT can be effective, but patients often
dislike the side effects of ADM and there are high costs associated
with delivering psychological therapies. Consequently, waiting times
in Britain currently average six to nine months, and can be as long as
two years \parencite{haliwell_fundamental_2007}.

There is evidence that the efficacy of CBT for treating depression is
falling \parencite{johnsen_effects_2015}

"One important way to improve outcomes and reduce the disease burden is
by focusing on treatments of chronic and treatment-resistant depression,
and on preventing relapse." \parencite{cuijpers_psychotherapies_2015}

In depression, relapse is common and its likelihood increases steeply
with each depressive episode \parencite{haliwell_fundamental_2007}.

But, but ... precision. For example, MBCT is specifically effective
against depressive relapse when 3 or more previous episodes
\parencite{segal_mindfulnessbased_2012}
cite meta analysis and relationship to ADM
...
Depression, anxiety and physical health difficulties result from
unconstructive, repetitive thought \parencite{watkins_constructive_2008}
...
Reviewing the rumination literature, \textcite{smith_roadmap_2009}
note that rumination as a construct is robust but that ``there
is no unified definition [...] or standard way of measuring it''
\textcite[][p. 117]{smith_roadmap_2009}.

Two contrasting conceptualisations of rumination of rumination have been influential.
Different models suggest different treatments which are more or less effective in different circumstances.

\begin{itemize}
\item{Response Styles Theory}

Response styles theory \parencite[RST; ][]{nolen-hoeksema_responses_1991}
considers rumination to be particular response to depressed mood where
a person tends to focus on ``causes, consequences and symptoms of one's
negative affect'' \parencite[][p. 117]{smith_roadmap_2009} thereby
prolonging low mood and depressive episodes. A significant output of
RST is the gold standard trait measure of rumination, the Ruminative
Responses Scale (RRS) \parencite{treynor_rumination_2003}.

"the use of positive distractions has not consistently been correlated
with lower levels of depressive symptoms in correlational studies,
although dozens of experimental studies show positive distractions
relieve depressed mood." \parencite{nolen-hoeksema_rethinking_2008}

"lead dysphoric people to become significantly less depressed for a
short time" \parencite{nolen-hoeksema_rethinking_2008}
...
\item{Goals and current concerns}
...
construed, negative content. \todo{I'm only really testing this in one
(uncontrolled) context i.e. at home, and not controlling time of day, mood or degree of state rumination.}

Worry is a thought process with similar characteristics to
rumination. \textcite[][p. 10]{borkovec_preliminary_1983} define worry
as \enquote{a chain of thoughts and images, negatively affect-laden
and relatively uncontrollable}. To some extent, rumination and
worry are temporally distinct in that rumination tends to focus on
past events and is associated with depression, whereas worry tends
to focus on events in the future and is associated with anxiety
\parencite{papageorgiou_depressive_2004,watkins_comparisons_2005}.
However, both forms of RNT regularly occur in the same individual, and
their similarities, at least in non-clinical populations, suggest they may
share common underlying processes \textcite{watkins_comparisons_2005}.

Also may start past then go future? \parencite{mclaughlin_effects_2007} (see
\textcite{smith_roadmap_2009})

"Given that a broader conceptualization of rumination is proposed here,
specificity to depression cannot be assumed, and it is suggested that
multiple measures of outcome are appropriate." \parencite{smith_roadmap_2009}

Furthermore, rumination is linked to levels of depression, and has been
shown to predict the onset of depressive episodes, but also predicts
symptoms of anxiety \parencite{nolen-hoeksema_role_2000}. The current
study measures both worry and rumination because of this this lack of
clear distinction and the common comorbidity of anxiety and depression
\parencite{kessler_impairment_1999}.

\paragraph{Attentional bias}

"a causal relationship between rumination and positive attentional bias
has been empirically established for the first time." \parencite{morrison_role_2008}


\paragraph{Koster's disengaement hypothesis}
...
but hypothesise that one driver of RNT, particularly brooding (which
is a subscale of the RRS \parencite{treynor_rumination_2003}), is an
impaired attentional ability to disengage from negative thoughts
\textcite{koster_understanding_2011}.  \todo{signpost lack of
demonstrating attentional engagement}

A cognitive correlate of worry is a disruption in attention-focusing
ability \parencite{borkovec_preliminary_1983}.

\paragraph{Cognitive bias modification}
...
counterconditioning mechanism which \textcite{watkins_habitgoal_2014}
 suggest may reduce RNT.


"we stressed the critical need to distinguish the question of whether a
putative ABM training procedure successfully induces the intended pattern
of selective attentional responding to threat, from the quite different
question of whether successful modification of selective attentional
responding to threat significantly alters dispositional and dysfunctional
anxiety." \textcite{macleod_attentional_2015}.

A double-blind, randomised controlled trial of ABM in individuals having
mild to severe depressive symptoms \textcite{yang_attention_2015} met both
criteria, demonstrating a post-training change in attentional bias which
mediated reductions in rumination and depressive symptoms. Furthermore,
reductions in depressive symptoms was mediated by rumination
\parencite{yang_attention_2015}.

\textcite{yang_attention_2015} used laboratory-based ABM training using
sad word stimuli. The proposed study aims to extend these findings by
testing whether ABM reduces RNT and elevates mood in non-depressed,
high ruminators exposed to abstract, negative, ideographic cue words
associated with an incomplete self-relevant goal.

"reduction in rumination is an effective treatment component of ABM
training on depression" \parencite{yang_attention_2015}.

This is a novel and more direct test of RNT within the habit-goal
framework \textcite{watkins_habitgoal_2014}, as this arrangement
should create a context likely to trigger RNT within which the
counterconditioning effects of ABM training can be measured.  \todo{Surely
this is more a test of Martin \& Tesser than the habit-goal hypothesis}

\todo{GAP: effects of ABM on current concerns control theory hasn't been tested}

A novel intervention to reduce rumination and treat anxiety and depression.

\paragraph{RAT/WTD}

Because the 
Anxiety and depression were measured due to their close association, and
uncertainty whether the procedure would affect rumination, worry or both.
...
\item Baseline state rumination, state anxiety and depression worry change after
ABM training.
\item Baseline trait rumination, trait anxiety, depression and worry
...
intervention (phase B) \parencite{bulte_r_2008}. The overall design can
be considered a nonconcurrent, multiple baseline replication across
participants \parencite{watson_nonconcurrent_1981}. That is to say,
there were muliple participants (case-series), which started at different
times, and the time point of transition to phase B in each was chosen
randomly in advance. Whilst the design lacks a control condition, it
does provide controls against alternative explanations of effects, such
as regression to the mean (replication across participants), effects of
baseline assessment (range of phase A durations) and extraneous events
correlated with phase transition (randomised transition to phase B)
...
The dot-probe task is commonly used to measure attentional bias,
based on differences in response times to pairs of stimuli
with differing affective valence, such as negative and neutral
word pairs \parencite{macleod_attentional_1986}. Our study used
word-pair stimuli (see figure \ref{fig:dotprobe}) and was based
on \textcite{yang_attention_2015}. We used two sets of visual word
stimuli in the dot-probe task. A set of Negative-Neutral (N-word)
pairs based on affective norms \parencite{citron_how_2014}, and a set
of Ideographic-Neutral (I-word) pairs associated with a current concern
...
correct response was recorded if the keypress matched the probe
...
eliminating any dependencies on a webserver for software development or
...
\subsubsection{Attentional Bias Modification task}

The Attentional Bias Modification (ABM) task used the same paradigm as the
dot-probe task but was designed modify, rather than measure attentional
bias. As such, it differed in three respects. First, there were double
the number (192) trials in each block. Second, only N-word pairs were
used as stimuli. Finally, probes in ABM trials always appeared at the
neutral word location. Thus, responses required repeated orientation of
attention away from the negative word in the pair. Each ABM block took
...
with each item in the range 1--9 \cref{app:concernrating}. The concern
...
instructed to replace any invalid words with synonyms or wth alternatives
...
approximate time when they would complete each session. If requested, the
experimenter installed OpenSesame \parencite{mathot_opensesame_2011} on
the participant's laptop. The initial session ended with the experimenter
and participant agreeing a time on the same or the following day when
the experimenter could be available to support the participant whilst
installing the computer task on their own computer and completing their
first session. All participants successfully completed the preparatory
session (see \cref{app:initialsession} for full details of the session
protocol).

The experimenter then prepared the files required for the
participant to run the computer task. These consisted of the
dot-probe/ABM program, I-word pairs, N-word pairs, the participant's
randomization schedule\footnote{Randomization schedules were
generated using the \texttt{SCRT} \parencite{bulte_r_2008} function
\texttt{selectdesign(design=AB,MT=35,limit=8)}.}, a stimulus
...
participant for installallation.
...
conern was still unresolved and complete PANAS-SF and GRS measures. On
...
longer task when transitioning to phase B. From session 25 onwards,
sessions ended with a message informing the participant of the amount
they had earnt so far. Participants provided post-intervention RRS,
PHQ-9, GAD-7 and PSWQ measures using the Lime Survey website within
12 hours of completing the final session. On exiting the study, all
...
the participant's ideographic words was measured at every session, whereas
the more general negative valence associated with N-words was distributed
across measurment times. In phase B, dot-probe measurement was preceeded
by ABM blocks containing only N-words. To detect ${generalised}$
...
any preceding ABM block. To guarantee enough stimuli when randomization
...
block. \cref{tab:allocation} shows an example stimulus allocation.
...
testing period. The experimenter mitigatated this risk by keeping in
...
\subsubsection{Pre--Post comparisons}
...
anxiety (GAD-7) and worry (PSWQ). Given the current design, paired
samples t-tests are likely to be underpowered, therefore the primary
analyses were visual inspection of the case-series plots and associated
inferential statistics from the randomization tests.  However, using a
similar ABM procedure, \textcite{yang_attention_2015} found significant
post training reductions in rumination, using RRS (${d}$=.49), depression,
using BDI-II \parencite{beck_manual_1996} (${d}$=1.50) and anxiety, using
STAI-T \parencite{spielberger_manual_1983} (${d}$=.88). Given these effect
...
were undergraduates with mild to severe depression in contrast with
the current study where participants with mild depression or above were
excluded. Estimates could also vary due to the different instruments used
to measure depression (PHQ-9) and anxiety (GAD-7) in the current study. In
an ABM training study using smartphones, \textcite{enock_attention_2014}
found a significant post ABM training reduction in PSWQ (${d}$=.51) using
a dot-probe task with neutral and disgust face stimuli. A two-tailed
...
Given the lack of power for some of the pre-post comparisons,
the primary analyses were visual inspection \todo{cite} and
randomization tests comparing A and B phases for measures of
attentional bias (I-words and N-words), state rumination (GRS)
and mood (PANAS-SF). A visual analysis was carried out across
time-series graphs of these measures generated by the R package \texttt{SCVA}
\parencite{bulte_when_2012}. Inferential statistics and effect sizes of
percentage of nonoverlapping data \parencite[PND,][]{scruggs_pnd_2013}
were calculated for each case using randomization tests provided by
the R package \texttt{SCRT} \parencite{bulte_r_2008}.

Randomization is a nonparametric approach which allows
inferential statistics to be calulated single-case series designs
\parencite{bulte_r_2008}. In an AB design, the phase sequence
(baseline followed by intervention) is fixed, therefore it
is the point at which phase A transitions to phase B which is
randomized. Under the null hypothesis, a test statistic for a
particular set of observations would not differ regardless of where
the transition from A to B occurs. Randomization tests hold the
...
observed test statistic \parencite{onghena_customization_2005}. In
addition to the single-case randomization tests, a meta-analysis
\parencite[][p.64]{onghena_customization_2005} for each outcome across
all participants was carried out using the R package \texttt{SCMA}
\parencite{bulte_singlecase_2013}. This allowed for significance
testing across participants above the ${p}$=.05 level, even though
the randomization tests could not detect significance at this level
for individual participants completing less than 35 sessions. The
meta-analysis used an additive approach, which has a greater probability
of yielding significant results when there are treatment effects
\parencite{edgington_additive_1972}.

Hmmm, perhaps N-word bias change should be directional but I-word and other measures non-directional, need to tie this back to hypotheses and fix analyse.R .  Change in scores and associated test statistic needs to be crystal clear in text and tables
Randomization test statistic used was \(\lvert A-B\rvert\) i.e. non-directional/2-tailed ...  \parencite{bulte_r_2008}.

Of the items added to the PANAS-SF, `sad' and `depressed' correlated
...
Three participants withdrew from the study. One person withdrew before
completing any sessions due to concerns that the study would worsen their
social anxiety, another found the time commit too great after 5 sessions,
and one withdrew after 24 sessions when a holiday meant that they would
be unable to complete sufficient phase B sessions.  For the remaining
participants, dot-probe/ABM accuracy was above the a priori inclusion
threshold of 75\% (see \cref{tab:completed}) and the concern chosen by
each participant at the preparatory session remained unresolved througout
the study. Session 32 for participant 7 was omitted from the analyses
as the dot-probe data file for this session was truncated. Session 8 GRS
and PANAS scores for participant 16 were brought forward from session 7,
...
Windows to Macintosh computer from session 21 onwards, and participant
19 completed later sessions in a different country from the one in which
they began the study.

\todo{Predictability in relation to screening measures
"Looking at the scores from these participants they were typical/atypical."
}
...
There were no outliers (> 3SD) on GAD-7, PHQ-9, PSWQ or RRS scores before
or after ABM training. There were no significant pre-post ABM training
differences (all ${P}$s {\textgreater} .05) on any of these outcomes
(see \cref{tab:prepost}). But RRS and PSWQ show moderate effect sizes ...
...
\todo{Point out that these are trait measures.}

\subsection{Visual Analysis}

Introductory paragraph on visual analysis
Will present each participant's day by day data.
This allows you to analyse things visually.  

\todo{Explain what people look for (cite): variability within each phase, change from A to B, trend in baseline that complicates interpretation ...  This is what to look out for ...}

\todo{Foreach DV: a couple of sentences for each.  Mainly a synthesis across Ps (patterns), but possibly draw attention to outliers.}

\subsubsection{Attentional bias N-words}
...
\caption{Scale range. Attentional bias scores for N-words (participants 5--10). A =
Phase A (dot-probe baseline), B = Phase B (ABM training followed by
dot-probe). Dotted lines show mean attentional bias score.}
\end{figure}

Description of these graphs.
...
\caption{Attentional bias scores for N-words (participants 11, 12, 15, 16, 18 and 19)}
\end{figure}

Description of these graphs.

\subsubsection{Attentional bias I-words}
...
\caption{Attentional bias scores for I-words (participants 5--10). A =
Phase A (dot-probe baseline), B = Phase B (ABM training followed by
dot-probe). Dotted lines show mean attentional bias score.}
\end{figure}
...
\caption{Attentional bias scores for I-words (participants 11, 12, 15, 16, 18 and 19)}
\end{figure}
...
\caption{FIXME: Scale range. GRS scores (participants 5--10). A = Phase A (dot-probe
baseline), B = Phase B (ABM training followed by dot-probe). Dotted
lines show mean GRS score.}
\end{figure}
...
\caption{GRS scores (participants 11, 12, 15, 16, 18 and 19)}
\end{figure}
...
\caption{FIXME: Scale range. PA scores (participants 5--10). A = Phase A (dot-probe
baseline), B = Phase B (ABM training followed by dot-probe). Dotted
lines show mean PA score.}
\end{figure}
...
\caption{PA scores (participants 11, 12, 15, 16, 18 and 19)}
\end{figure}
...
\caption{FIXME: Scale range. NA scores (participants 5--10). A = Phase A (dot-probe
baseline), B = Phase B (ABM training followed by dot-probe). Dotted
lines show mean NA score.}
\end{figure}
...
\caption{NA scores (participants 11, 12, 15, 16, 18 and 19)}
\end{figure}
...
\caption{FIXME: Scale range. Depression ('sad'+'depressed') scores (participants 5--10). A
= Phase A (dot-probe baseline), B = Phase B (ABM training followed by
dot-probe). Dotted lines show mean depression score.}
\end{figure}
...
\caption{Depression ('sad'+'depressed') scores (participants 11, 12, 15, 16, 18 and 19)}
\end{figure}
...
\begin{itemize}
\item Could individual's current concern(s) indicate the type of bias modification which might be beneficial?
...
\begin{itemize}
\item Why didn't I measure change in the RRS brooding subscale?
\item self-report measures
\item Longevity of effects treatment? Would ABA revert?
...
\subsubsection{RFW}
\begin{itemize}
\item Inhibitory control \parencite{koster_understanding_2011}.
\end{itemize}

\paragraph{multiple baselines}

Didn't do this analysis due to differing numbers of sessions.
...
could provide oppotunities for more precise measurement and personalised
treatments using experience sampling/EMA. Especially with case-series
approach and randomization tests.

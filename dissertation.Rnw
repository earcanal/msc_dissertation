...
disability-adjusted life years (DALY). Based on 2007 prevalance rates,
...
psychological treatments, has response reates of 50\% or less for
...
real-time monitoring of mood and behavior could also contribute
\parencite{cuijpers_psychotherapies_2015}. More reasearch is required
...
subscales which can distinguish maladaptive rumination in the form of
...
Goal progress theory \parencite{martin_ruminative_1996} is a
more general model which proposes that rumination is initiated
...
model to explain discrepancies arising amongst concurrent goals
\parencite{martin_extending_2006}.}. This account of rumination draws
on ideas from control theory \parencite{carver_control_1982} such that
feedback mechanisms orient cognitive resources towards maintaining
a system within desirable constraints. The system in this case is an
individual's hierarchy of goal representations, unexpected progress
indicates unnmet constraints and the cognitive resources allocated to
address the discrepancy include attention, memory and consciousness. A
concept closely related to goal progress is the 'current concern', which
\textcite[][p.249]{klinger_motivation_2011} define as \enquote{the state
of an organism between the time it becomes committed to pursuing a goal
and the time it either gains the goal or abandons the pursuit.} Rumination
for \textcite[][p. 7]{martin_ruminative_1996} is "a class of conscious
thoughts that revolve around a common instrumental theme and that recur
in the absence of immediate environmental demands requiring the thoughts",
and such thoughts are a consequence of goal discrepancies. Together, goal
progress theory and current concerns provide a framework for understanding
rumination in terms of motivation, cognition and emotion. Where progress
towards resolving a current concern is slower than expected, priming will
potentiate internal and external cues on related themes to draw attention
to the discrepancy. Similarly, negative affect will draw attention to
the unsatisfactory goal progress. This account explains why targetting
underlying goal discrepancies may provide more long term relief than
distraction interventions in relieving the negative affect associated
with rumination.

\paragraph{Worry is a thought process with similar characteristics to
...
\textcite{klinger_motivation_2011} 

"It remains unclear if rumination is best characterized as an automatic
or consciously controlled process" \textcite{smith_roadmap_2009}

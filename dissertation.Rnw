...
time on the same or the following day when the participant would complete
the software installation on their own computer and complete the first
session. All participants successfully completed the preparatory session
(see \cref{app:initialsession} for full details of the session protocol).
...
height. These files were emailed to the participant, along with a link
to the installation video on YouTube.

\subsubsection{Daily Sessions}

Participants were instructed to complete all experimental sessions on a computer at their home.
...
minutes for training blocks and self-report measures).
...
\subsubsection{Final session}

Participants provided post-intervention RRS, PHQ-9, GAD-7 and PSWQ measures using the Lime Survey website 

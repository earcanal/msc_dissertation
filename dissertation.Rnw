...
and the cost-effectiveness of ABMT as psychological therapy evaluated.}
...
temporally separating the stimulus from the probe, the dot-probe may be
...
the hypothesised directions. Also unsupported were our hypotheses that
...
the current study may have more effectively measured and modified
...
rather than a ``goal''. Finally, for consistency with previous research
...
but existing tasks have a limited ability to validate them. Future
work should look for approaches which improve on the Stroop and
dot-probe tasks as measures and modifiers of attentional bias.

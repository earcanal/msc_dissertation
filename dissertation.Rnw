...
dim(outcomes) <- c(4,3)
formatted <- data.frame(1:4,1:4,1:4,row.names=outcomes[,1])
colnames(formatted) <- c('ct_pre','ct_post','d')
tests <- apply(outcomes, 1, function(o) {
  ...
  t.test(prepost[[post]], prepost[[pre]], paired=TRUE, var.equal=TRUE)
})
...
# FIXME: report CIs? Does t.test produce 95% CIs?

...
database \parencite{wilson_mrc_1988}\todo{do I need more details on
where the MRC attributes come from?}. A separate table was added to the
...
initial session ended with the experimenter and participant agreeing
a time on the same or the following day when the participant would
complete the software installation on their own computer and complete
the first session.

Amazingly this worked first time for all Ps. Full details of the protocol are in \cref{app:initialsession}

The experimenter then created the files required for the
participant to run the computer task.  These included the
experiment itself and a number of data files. The data files
consisted of the I-word pairs (\cref{sec:iwordpairs}) which were
validated by the experimenter's supervisor, the N-word pairs, the
participant's randomization schedule (generated using the command
...
height. These files were combined in a zip file which was emailed to
the participant, along with a link to the installation video on YouTube.
...
This study was approved by the ... (see \cref{app:ethics})

...
pval <- function(x) {
  ...
  if (round(x, digits=2) < 0.001) {
    p <- '< .001'
  } else {
    x <- sprintf("%0.3f",x)
    p <- str_replace(as.character(x), "^0\\.", ".")
  }
  c(sig,p)
}
# FIXME: include CIs?
# FIXME: check assumptions
p   <- pval(results['RRS','p.value'])
rrs <- sprintf("There was %s significant difference between pre (${M}$=%0.2f, ${SD}$=%0.2f) and post (${M}$=%0.2f, ${SD}$=%0.2f) RRS scores, with ${t}$(%d) = %0.2f, ${p}$ = %s",p[1],mean(prepost$rrspre),sd(prepost$rrspre),mean(prepost$rrspost),sd(prepost$rrspost),results['RRS','df'],results['RRS','t'],p[2]) 

library(xtable)
strCaption <- paste0("\\textbf{Table n} Pre-post comparisons")
print(xtable(df, digits=2, caption=strCaption, label="prepost"), 
      size="footnotesize",
...
\caption{Pre-post Comparisons}

\begin{tabular}{llllll}
\toprule
Participant & Sex & {$M$}({${SD}$}) & {$t$} & {$p$} & {$d$}\\
\midrule
5 & F & m(sd) & t & p & $\pi=\Sexpr{pi}$ \\
\midrule[\heavyrulewidth]
%\multicolumn{6}{l}{$M=\text{mean}$, ${SD}=\text{standard deviation}$}
\end{tabular}
\end{table}

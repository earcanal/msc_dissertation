...
\todo{recruitment flowchart, how many: responsed to advert, met
screening criteria, dropout and its predictability in relation to
screening measures}
...
\todo{mean/SD: rate of completion, sessions skipped, dropout and its predictability in relation to screening measures}
...
\todo{table with P sex M(SD) t p d * PHQ-9, PSWQ, GAD-7, RRS}
...
dim(outcomes) <- c(4,3)
foo <- data.frame(1:4,1:4,1:4,row.names=outcomes[,1])
colnames(foo) <- c('ct_pre','ct_post','d')
tests <- apply(outcomes, 1, function(o) {
  ...
  # t-tests (recipe 9.15); Student's t-test: var.equal=TRUE
...
results <- sapply(tests, function(x) {
  c(x$statistic,x$parameter,p.value = x$p.value,ci.lower = x$conf.int[1],ci.upper = x$conf.int[2])
})
results <- t(results) # put outcome measures in the rows
...
format <- function(x) {
  ...
  cil  <- sprintf("%0.2f",x['ci.lower'])
  ciu  <- sprintf("%0.2f",x['ci.upper'])
  x['ct_pre']  <- pre
  x['ct_post'] <- post
  x['df']      <- df
  x['t']       <- t
...
results <- t(results)
results <- subset(results,select = c('ct_pre','ct_post','t','df','p.value','ci.lower','ci.upper','d'))

# FIXME: include CIs?
# FIXME: check assumptions
words <- apply(results,1,function(r) {
  #sprintf("There was %s significant difference between pre (${M}$=%0.2f, ${SD}$=%0.2f) and post (${M}$=%0.2f, ${SD}$=%0.2f) RRS scores, with ${t}$(%s) = %s, ${p}$ = %s, Cohen's ${d}$ = %s.",'a/no',1,2,3,4,r['df'],r['t'],r['p.value'],r['d'])
  sprintf("There was %s significant difference between pre (${M}$(${SD}$) = %s) and post (${M}$(${SD}$) = %s) RRS scores, with ${t}$(%s) = %s, ${p}$ = %s, Cohen's ${d}$ = %s.",'a/no',r['ct_pre'],r['ct_post'],r['df'],r['t'],r['p.value'],r['d'])
})

library(xtable)
strCaption <- paste0("\\textbf{Table n} Pre-post comparisons as a table")
print(xtable(results, caption=strCaption, label="prepost"), 
      size="footnotesize",
      ...
      add.to.row = list(pos     = list(-1, nrow(results)),
                        command = c(paste("\\toprule \n",
                                          "& Pre & Post\\\\\n", 
                                          "Measure & ${M}$(${SD}$) & ${M}$(${SD}$) & ${t}$ & ${df}$ & ${p}$ & CI(lower) & CI(upper) & d\\\\\n", 
                                          "\\midrule \n"),
                                          "\\bottomrule \n")
                        )
)
...
Limitation: relatively coarse (trait) measures.  Only 2 measurement times (pre/post) vs. daily changes in AB, mood, rumination.
...
\todo{table with P sex M(SD) p PND}

\subsubsection{Meta analysis}

Ecological validity: daily measures in real environment.

\section{Discussion}

Limitation: how long lasting is treatment?
Would ABA revert?
What happens when problem resolves?

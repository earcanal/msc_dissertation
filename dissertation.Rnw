...
31.38 years (${SD}$ = 9.99 years). Three participants withdrew from
...
included a link to the installation video on YouTube. Each session
required the participant to complete the dot-probe task (phase A)
or the ABM training, followed by the dot-probe (phase B) and online
PANAS-SF and GRS measures using Lime Survey. On a daily basis, the
experimenter recorded whether each participant had completed a session
and, if so, their accuracy on the task. The experimenter periodically
sent additional emails to motivate participants by notifying them of
their progress, to ensure their well-being if 3 consecutive sessions
were skipped, and to prepare them for the slightly longer task when
transitioning to phase B. From session 25 onwards, sessions ended with
a message informing the participant of the amount they had earnt so
far. Participants provided post-intervention RRS, PHQ-9, GAD-7 and PSWQ
measures using the Lime Survey website within 12 hours of completing
...
no problems resolved (mention that they recorded this in Procedure)

paragraph on included Ps
...
How many did each P complete and why did they stop.

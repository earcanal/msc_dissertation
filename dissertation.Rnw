...
attention from self-referent negative thoughts, and that reducing
this bias will reduce rumination and depressive symptoms. A broader
theory proposes that rumination is the result of discrepancies between
perceived and expected progress towards goals. Current concerns are
mental states representative of such goal progress discrepancies. The
current study tests the disengagement hypothesis within this goal
progress theory of rumination. It aims to reduce attentional bias to
towards negative words and words associated with an unresolved current
concern, and detect corresponding reductions in rumination, worry, mood
and symptoms of anxiety and depression. Participants were 12 (10 female)
university students or staff scoring low for depressive symptoms but
high for trait rumination and/or trait worry. Attentional bias was
measured using a dot-probe task, and attentional bias modification
training (ABMT) delivered using a variation of the same task. Stimuli
were negative--neutral word pairs, and neutral words paired with words
associated with a current concern chosen by each participant. Using
a multiple baseline case-series design, each participant completed
between 25--35 dot-probe sessions at home, a minimum of 8 also including
ABMT. There were no significant post-training effects on trait rumination,
trait worry, and symptoms of depression and anxiety. Randomisation
tests showed no significant post ABM reduction in attentional bias
towards negative words, attentional bias towards current concern words,
rumination towards the concern, positive affect, negative affect or
depressive symptoms. The limitations of the dot-probe task are discussed
and the cost-effectiveness of ABMT as pscyhological therapy evaluated.}
...
interference between naming a word's colour, and affective or cognitive
bias resulting from reading the word. First, concurrent presentation
...
and post ABMT measures of rumination (RRS), depression (PHQ-9),
...
Comparisons between outcome measures pre and post ABMT
...
ABMT differences (all ${P}$s {\textgreater} .05) on any of these
outcomes.
...
This study predicted that ABMT would reduce attentional
...
ABMT would reduce attentional bias towards words associated with
an ideographic current concern, and rumination towards this concern.
These findings do not support the hypothesis that rumination occurs as
...
\parencite{yang_attention_2015} that ABMT can reduce attentional
...
to detect post-ABMT outcomes. \textcite{yang_attention_2015}
...
The dot-probe is a relatively new psychological task, and questions
remain regarding how stimulus and population characteristics
...
symptoms and rumination. Shorter word presentations 1250 ms in
the current study may have more effectivel measured and modified
bias, given that the current sample scored higher on worry than
rumination. Stimulus type (words vs pictures) and sample characteristics
(healthy vs. high symptomatology) are known to modulate the effects
of ABM on attentional bias, but their interactions remain unclear
\parencite{beard_efficacy_2012}. Questions also remain about interactions
between stimulus semantics and sample characteristics. Using the Stroop
task, \textcite{mogg_subliminal_1993} found that anxious participants
were slower than depressed and normal participants at naming negative,
but not generally emotional words. The reductions in rumination and
...
ABMT using sad and neutral words. Depression-congruent bias in the
Stroop task has been shown to depend on self-referent elaborative
processing \parencite{nunn_selective_1997}. In the current study,
the I-words were highly self-relevant, but the ABMT only included
N-word pairs, so it is possible that words with normative, negative
valence were not sufficiently self-relevant for ABMT to modify bias
in this particular sample.

Mixed findings across studies which use the dot-probe and/or
ABMT could be also be explained by its poor internal consistency
and test-retest reliability, which has been demonstrated in
...
\parencite{kappenman_behavioral_2014}. Future research should try to
identify a more reliable means of measuring and modifying attentional
bias. The dot-probe may also be unsuitable for studying attentional biases
...
exposers are required to detect biases associated with depression and
...
worry and rumination, and this limitation of the dotprobe complicates the
...
with the mixture of valences in participants I-word ratings (see
...
can occur in the absence of goal discrepancies, notably in the
case of worry. A related framework with more directly testable
hypotheses \parencite{watkins_habitgoal_2014}, suggests that while
goal discrepancies may cause rumination, this can become a habitual
thinking style. Within this framework, rumination with no overt goal
discrepancy may instead be cued by contingent contexts such as mood,
or the physical location in which it tends to occur. Rumination is
especially likely when contingent responses involve passive focus on
abstractly construed, negative content. The disengagement hypothesis
\parencite{koster_understanding_2011} is especially suited to, and
as yet untested, within this framework. If rumination is a habitual
thinking style, the repetitive nature of ABMT seems promising as
a counter-conditioning technique. The current study used ABMT in a
...
people would be unlikely to maintain ABMT at this level of intensity.
...
depression. \parencite{hallion_metaanalysis_2011} found that CBM
significantly modified anxiety but not depression, with nonsignificant
...
and CBM targetting interpretation (CBM-I) does indeed show larger
...
with sufficient statistical power. Nevertheless, the imminent emergence
...
disorders. Future research in this are may explain why bias modification
...
This was also an exploratory study, so limiting Type I error rates was not
a priority and a control condition could be added to a subsequent study
there been significant findings. The study would have benefitted from a
...
rather than a ``goal''. Finally, post ABMT stressor effects required to detect effects in previous studies 

\parencite{see_reduction_2009}

\subsection{Conclusion and RFW}

...
disability-adjusted life years (DALY). Based 2007 prevalance rates,
...
(I-CBT). Despite their strong evidence-base, pharmacological and
psychological treatments are only partially effective. Combinations
of these treatments can only reduce the disease burden of depression
by about 33\% \textcite{andrews_utilising_2004}, and CBT, the gold
standard psychological treatments, has response reates of 50\% or less
for treating anxiety disorders \parencite{schneider_state_2015}. There
is also evidence that the efficacy of CBT for treating depression is
falling \parencite{johnsen_effects_2015}, and the high costs associated
with delivering psychological therapies mean that waiting times in
Britain currently average six to nine months, and can be as long as
two years \parencite{haliwell_fundamental_2007}. Technological advances
are beginning to improve cost-effective treatment delivery. I-CBT is a
viable treatment for adults with depression and some anxiety disorders
\parencite{arnberg_internetdelivered_2014} and mobile apps including
real-time monitoring of mood and behavior could make a major contribution
\textcite{cuijpers_psychotherapies_2015}. More reasearch is required
to ascertain whether this potential can be realised in practice. A
second area of progress is an improved precision of treatments, based
on an expanded understanding of the role played by cognitive mechanisms
such as attention and memory in mood disorders. For example, Mindfulness
Based Cognitive Therapy (MBCT) is a treatment which specifically targets
...
and compter tasks offer cost-effective means of treatment delivery.

\paragraph{Rumination}

Boom - cognitive mechanisms ...

Important to stress that rumination is normal and can be non-pathalogical e.g. problem solving, reflective pondering 
...
and it's this type of rumination that we're talking about here.
...
Maybe list the different models to highlight the lack of agreement
...
\item{Goal Progress Theory}

A different mechanism that is frequently the cause of repetitive thought.
...
implicit/explicit motivation? \parencite{schultheiss_role_2008}
endogenous/exogenous goals \parencite{klinger_motivation_2011}
...
cf. \textcite{martin_ruminative_1996}
\end{itemize}

POC:Regarding distraction "As long as the goal remains desired yet
unattained, the mental conditions that promote rumination will persist"
...
"the results provide direct support for the hypothesis that similar
cognitive control impairments may play a role in worrying" \parencite{beckwe_worrying_2014}
...
Understanding the mechanisms is the way forward.

Proposed mechanisms include
Inhibitory control \parencite{koster_understanding_2011}.

" rumination was related to attentional switching impairments,
specifically in the context of emotional information" \parencite{koster_rumination_2013}
...
GAP: effects of ABM on current concerns/goal progress theory hasn't been tested
...
can be considered a nonconcurrent, multiple baseline replication
across participants \parencite{watson_nonconcurrent_1981}. That is
to say, there were muliple participants (case-series), which started
...
individual regression to the mean, variable phase A durations controls
...
and PANAS-SF scores for participant 16 were brought forward from session 7,
...
Each participant's session data

Explain how visual analysis is normally done: variability within each phase, change from A to B 'level', trend in baseline that complicates interpretation 

"The broadened median summarizes the data in a way which balances the twin
goals of being highly sensitive to a reasonable proportion of the data,
while being resistant to outliers." \parencite{morley_graphical_1991}
...
\caption{Attentional bias scores for N-words (participants 5--10). A =
Phase A (dot-probe baseline), B = Phase B (ABM training followed by
dot-probe). Solid horizonal lines show mean attentional bias score. Dashed
lines show trend (least squares regression) within each phase.}
\end{figure}

Describe graphs. Mainly a synthesis across Ps (patterns), but possibly draw attention to outliers.
...
ROF:"In contrast with current theories, we found that 34\% of participants
...
others." \parencite{zvielli_dynamics_2014}

Perhaps this is reflected in my results for worriers? Hmm, were they "threatened" by their concern?

\subsection{Thoughts}

*Causality* is still unclear and under researched so RFW.
...
this sample. While they calim the rebound effect of thought suppression
...
RFW: Establish the likely cause(s) before testing potential
antidotes! Also need to subsequently match the right treatment. Maybe
...
Case-series is a good, quick way of testing things out?

...
\parencite{wilson_mrc_1988}\todo{do I need more details on where the
MRC attributes come from?}. A separate table was added to the database
...
People meeting the screening criteria where invited to participate,
and those who agreed met the researcher at the University of Exeter to
complete the initial session. After giving informed, written consent,
the participant used a Lime Survey website \parencite{limesurvey} to
complete baseline RRS, PHQ-9, GAD-7 and PSWQ measures.

Using a procedure based on \textcite{roberts_cueing_2013}, the
participant was instructed to select an unresolved current concern
(see \cref{sec:iwordinstructions}).
...
, before being introduced to the experimental

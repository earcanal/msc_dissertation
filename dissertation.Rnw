...
thoughts or external cues \parencite{williams_cognitive_1988}. One
...
If cognitive biases underly emotional disorders, then
cognitive bias modification (CBM) offers potential treatments
\parencite{hertel_cognitive_2011}. CBM often uses computer tasks to
modify biases of attention, interpretation or memory. The dot-probe
...
to reduce depressive symptoms \parencite{yang_attention_2015}. Critics of
CBM argue that the effect sizes are too small to be clinically relevant
\parencite{cristea_efficacy_2015}. However, advocates argue that
this critique is based on a meta-analysis which didn't differentiate
ABM from other forms of CBM, didn't focus on a single psychological
dysfunction (anxiety), or didn't analyse studies which demonstrate
both attentional bias change ${and}$ corresponding symptom change
\parencite{macleod_attentional_2015}. It should be noted that in
...
study measures the effects of ABM on daily rumination and mood,
and on measures of rumination, worry, anxiety and depressive symptoms over
longer periods. Finally, the study aims to extend the findings of
\textcite{yang_attention_2015} by testing whether the ABM can reduce
rumination and depressive symptoms in a non-clinical sample. Specific
...
Did it work for some but not others? e.g. P11 sig. on N-words, large GRS PND
...
\subsubsection{Better therapies}
...
But why are existing therapies relatively ineffective?
...
\item Dose response? trend reversal in NA for p8 but maybe
not enough sessions (but loads and people won't keep them up!)

\item Longevity of effects treatment? Would ABA revert? Problem as

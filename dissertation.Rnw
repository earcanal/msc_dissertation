...
hypothesis arising from the attentional bias literature is that
depressive rumination results from an impaired ability to disengage
attention from negative thoughts, and that reducing this bias will
reduce rumination and depressive symptoms. The goal progress theory
of rumination proposes that rumination is the result of discrepancies
between perceived and expected progress towards a goal. Current concerns
are mental states representative of such goal progress discrepancies. The
current study tests the disengagement hypothesis by aiming to reduce
attentional bias to towards negative words and words associated with an
unresolved current concern, and measuring the effects on rumination,
worry, mood and symptoms of anxiety and depression. Participants were
12 (10 female) university students or staff scoring low for depressive
symptoms but high for trait rumination and/or trait worry. This was a
non-concurrent, multiple baseline case-series replication in which each
participant completed between 25--35 sessions, a minimum of 8 including
ABM training. Participants ran dot-probe tasks on their own computers
designed to first measure, then modify attentional bias towards a
general set of negative words and words associated with a current
concern of their choosing. There were no significant post-training
effects on trait rumination, trait worry, and symptoms of depression and
anxiety. Randomisation tests showed no significant post ABM reduction
in attentional bias towards negative words, attentional bias towards
current concern words, rumination towards the concern, positive affect,
negative affect or depressive symptoms.}

\authornote{Acknowledge}
...
are commonly co-morbid \parencite{kessler_impairment_1999}. For
example, mixed depression and anxiety were reported as being
...
psychological therapy wait less than 18 weeks for treatment
...
depression and anxiety, from more constructive forms of repetitive thought
which support planning, problem solving, and healthy self-reflection
\parencite{watkins_constructive_2008}.  Whilst there is broad consensus
regarding the role playted by rumination, the ten models and five
measures reviewed by \textcite{smith_roadmap_2009} demonstrate that
``there is no unified definition [...] or standard way of measuring
it'' \textcite[][p. 117]{smith_roadmap_2009}. Response styles theory
\parencite[RST; ][]{nolen-hoeksema_responses_1991} has been influential in
terms of explaining, measuring and treating depressive rumination. RST
...
distraction induction, in dysphoric but not in nondysphoric participants
...
\parencite{martin_extending_2006}.}. This account draws on ideas from
control theory \parencite{carver_control_1982} in that goal discrepancies
represent system states outside of desirable thresholds which cognitive
process try to restore through feedback mechanisms. Rumination
is made more likely as a result of this deployment of attention,
memory and consciousness, and will remain present until goal progress
becomes satisfactory, the goal is perceived as being met or it has been
abandoned. For \textcite[][p. 7]{martin_ruminative_1996}, rumination is
"a class of conscious thoughts that revolve around a common instrumental
theme and that recur in the absence of immediate environmental demands
requiring the thoughts". A concept closely related to unresolved goals is
the 'current concern', which \textcite[][p.14]{klinger_motivation_2011}
...
negative affect draws attention to the unsatisfactory goal progress
...
\textcite{mclaughlin_effects_2007} confirmed this general distinction,
however they also found that induced rumination began as past-focused
but then displayed a linear trend towards thoughts about the present
and finally the future. Whilst rumination is linked to depression
...
in non-clinical populations) \parencite{watkins_comparisons_2005}, and
...
associated with anxiety and depression may also play a
role in rumination. These include biases of attention, memory,
interpretation and emotional association, and inhibitory control deficits
\parencite{mathews_cognitive_2005}. Attentional bias is a disproportionate
attentional sensitivity to particular stimuli, be they internal
thoughts or external cues \parencite{williams_cognitive_1997}. One
...
of worry \parencite{borkovec_preliminary_1983}. It also
remains unclear to what extent attentional bias results
in rumination or rumination leads to attentional bias
...
If cognitive biases underly emotional disorders,
...
pairs. Participants respond to a probe which appears at one or the other
location of the previously displayed stimuli.  When measuring attentional
bias, the dot-probe has two advantages over variants of the Stroop
task, in which response time differences are considered to represent
interference between colour-naming of words and bias relating to cognitive
and/or affective properties of those words. First, concurrent presentation
(colour and word) in the Stroop makes it unclear whether competition is
between attention or response selection \parencite{macleod_half_1991}. By
temporally seperating the stimulus from the probe, the dot-probe may be
a more pure measure of attentional bias. Second, the dot-probe is easily
adapted to modify attentional bias, by manipulating the probe location
to train attention towards, or away from particular stimuli. Numerous
studies have indicated that ABM may be an effective treatment for anxiety
...
psychological dysfunction (anxiety), or didn't analyse studies which
demonstrate both attentional bias change ${and}$ corresponding symptom
change \parencite{macleod_attentional_2015}. It should be noted that in
demonstrating the mediating role of rumination in reducing depressive
symptoms, \textcite{yang_attention_2015} met both of these final criteria.
...
using the dot-probe, before testing the effects of ABM training designed
to reduce vigilance towards negative words, and words associated with
a current concern. This can be considered a test of the attentional
disengagement hypothesis \parencite{koster_understanding_2011}
in relation to the goal progress theory of rumination
...
rumination, \textcite{smith_roadmap_2009} recommends using multiple
outcome measures. The difficulties in distinguishing rumination from
worry, and the common co-morbidity of anxiety and depression also suggest
multiple measures are appropriate. The current study measures the effects
of ABM on daily rumination and mood, and on measures of rumination, worry,
anxiety and depressive symptoms over longer periods. This novel use of
ABM training offers the potential to offer a personalised, cost-effective
...
current concern will reduce after ABM training.
...
\item After ABM training, state rumination, negative affect and depressive
...
a relatively long-term training regime. A randomised, single-case
series design \parencite{bulte_r_2008} was chosen to allow statistical
inference in spite of this small ${N}$.  This type of design compares a
series of baseline (phase A) measures against those after a treatment
intervention\footnote{The treatment in the current study being ABM
training.} (phase B) \parencite{bulte_r_2008}. The overall design can
be considered a non-concurrent, multiple baseline replication across
participants \parencite{watson_nonconcurrent_1981}. That is to say,
there were multiple participants\footnote{A 'series' of 'cases'.}, who
started the study at different times, and each participant's transition
to phase B was chosen randomly in advance. Based on previous research
...
\parencite{limesurvey} and completed online. People with mild depression
or above (PHQ-8 \parencite{kroenke_phq8_2009} {\textless} 11) were
excluded. Inclusion criteria were high trait rumination (Ruminative
Responses Scale (RRS) \parencite{treynor_rumination_2003} ${\geq}$
53), and/or high trait worry (Penn State Worry Questionnaire (PSWQ)
\parencite{meyer_development_1990} ${\geq}$ 55). The RRS and PSWQ cut
...
score of \Sexpr{sprintf("%0.1f",mean(screening$phq8total))}
(${SD}$=\Sexpr{sprintf("%0.1f",sd(screening$phq8total))}), a mean
RRS score of \Sexpr{sprintf("%0.1f",mean(screening$rrstotal))}
(${SD}$=\Sexpr{sprintf("%0.1f",sd(screening$rrstotal))}), and a
mean PSWQ score of \Sexpr{sprintf("%0.1f",mean(screening$pswqtotal))}
(${SD}$=\Sexpr{sprintf("%0.1f",sd(screening$pswqtotal))}). There were no
...
a summary of which can be seen in \cref{app:ethics}.
...
\parencite{limesurvey} and completed using a web browser  Ecological validity was good, with all except the
...
dot-probe/ABM sessions (see \cref{app:surveys}).

\paragraph{RRS}

The Ruminative Responses Scale (RRS) is a 22 item measure
of trait rumination, with item scores ranging from 1--4. The
...
symptoms, with items relating to the previous 2 weeks, and scores ranging
from 0--3. The GAD-7 is an efficient, internally reliable (Cronbach's
$\alpha$ > .85) tool for assessing GAD severity in clinical research,
with good test-retest reliability \parencite{spitzerrl_brief_2006}.
...
towards a specific goal \parencite{schultheiss_role_2008}. Item scores
...
as the NA scale itself (see \cref{app:mood}). Given these findings and
...
People who met the online screening criteria and accepted the invitation
to participate, met the researcher at the University of Exeter to complete
...
completed baseline measures for RRS, PHQ-9, GAD-7 and PSWQ online.

Using a procedure based on \textcite{roberts_cueing_2013}, the participant
was instructed to select an unresolved current concern which they rated
on a 5 item scale, with each item in the range 1--9. The concern was
accepted if ratings for the questions \enquote{How much you have been
thinking about it over the last week?} and \enquote{How much does it
bother you now?} were > 5, and the concern was considered unlikely
to resolve within the following 5 weeks. For each participant, the
first concern chosen met these criteria. Participants wrote a short
statement summarising their concern and recorded the length of time
that it had been bothering them. The participant then generated their
I-words (see \cref{app:iwordgen}) and completed a dot-probe practice
block\footnote{Dot-probe practice blocks used N-word block 7 and
a set of I-word pairs generated by the experimenter.}. Participants
were given help installing the computer task, a task information sheet
(see \cref{app:taskinstructions}) and a printed calendar for managing
their daily sessions, on which they wrote an implementation intention
\parencite{gollwitzer_implementation_1999} to encourage session
completion. The experimenter then prepared the files required for the
computer task and emailed them to the participant.
...
(see \cref{app:reminder}). Participants began each session by completing
the dot-probe task (phase A) or the ABM training, followed by the
dot-probe (phase B). Dot-probe blocks took approximately 5 minutes to
complete, and ABM training blocks approximately 15 minutes.
...
GRS measures. The experimenter periodically sent additional emails
to motivate participants by notifying them of their progress, to
ensure their well-being if 3 consecutive sessions were skipped, and to
prepare them for the slightly longer task when transitioning to phase
B. From session 25 onward, sessions ended with a message informing the
participant of the amount they had earned so far. Participants provided
post-intervention RRS, PHQ-9, GAD-7 and PSWQ measures using the Lime
Survey website within approximately 12 hours of completing the final
session. On exiting the study, all participants were emailed debrief
information \cref{app:debrief} and information about sources of support
\cref{app:support} and were paid for their participation.
...
and number of syllables (see \cref{app:nwords}). Ideographic word sets
...
word length and number of syllables (see \cref{app:iwords}). In order
...
using screen capture software and uploaded to YouTube.} (see figure
...
\subsubsection{ABM training}

The ABM task used three minor adjustments to the dot-probe paradigm to
...
two-tailed, paired samples t-tests were used to compare participants' pre
and post ABM training measures of rumination (RRS), depression (PHQ-9),
anxiety (GAD-7) and worry (PSWQ). Whilst these tests were potentially
underpowered, it was noted that the significant post training effect sizes
found by \textcite{yang_attention_2015} were medium for rumination (RRS,
${d}$=.49), and large for depression (BDI-II \parencite{beck_manual_1996},
${d}$=1.50) and anxiety (STAI-T \parencite{spielberger_manual_1983},
${d}$=.88).

\subsection{Daily sessions}

Each participant's daily session data was analysed by comparing phase A
and B measures of attentional bias (I-words and N-words), state rumination
(GRS) and mood (PANAS-SF). Visual analyses were carried out using
time-series graphs generated by the R package \texttt{SCVA}\footnote{Minor
modifications were made to the \texttt{SCVA} package to standardise
y-axis ranges and to plot central tendency, trend and variance lines
on the same graph.} \parencite{bulte_when_2012}. Inferential statistics
were calculated for each participant using randomisation tests provided
by the R package \texttt{SCRT} \parencite{bulte_r_2008}.
...
observations would not differ systematically as a function of where
the transition from A to B occurs. Randomisation tests hold the
observation scores constant and calculate the test statistic for
all possible transition permutations. Once sorted, the ${p}$ value
...
the R package \texttt{SCMA} \parencite{bulte_singlecase_2013}. With 12
...
at the case level was below ${p}$=.05 for participants who completed
less than 35 sessions. The meta-analysis used an additive approach,
which has a greater probability of yielding significant results when
there are treatment effects \parencite{edgington_additive_1972}. The
percentage of nonoverlapping data (PND) is an effect size measure
frequently used in single-subject research, and is the percentage of
treatment phase observations that do not overlap data in the baseline
phase \parencite{scruggs_pnd_2013}. \textcite{scruggs_early_1986}
consider treatments with PND scores ${\geq}$ 90\% as highly effective,
scores of 70\%--90\% as fair, scores of 50\%--70\% as questionable
effects, and scores <~ 50\% as unreliable.  PND was calculated using the
R package \texttt{SCMA}, using \texttt{PND-} for N-words, I-words, NA and
depressive symptoms as the treatment should cause these measures to drop,
and \texttt{PND+} for PA as the treatment should result in an increase
in positive affect. All questions on the daily online outcome measures
were mandatory, so there was no missing data for any completed survey.
...
inclusion threshold of 75\% (see \cref{tab:completed}) and the current
concern chosen by each participant at the preparatory session remained
...
A common approach to visually analysing case-series is to examine
post-treatment changes in central tendency and linear trends
\parencite{morley_graphical_1991}. In the following graphs, the broadened
median (BMED) is used as a measure of central tendency as it is both
sensitive to a reasonable proportion of the data, and resistant to
outliers in relatively small samples \parencite{morley_graphical_1991}. We
expected to see visual differences in BMED in the hypothesised
direction for each outcome\footnote{On the attentional bias graphs,
the change in BMED between phases A and B is visually small relative
to the variability due to outliers for some participants.}. Plotting
linear trends in central tendency reduces visual bias when estimating
trends from raw data points (due to outliers). The trend lines
in following graphs were plotted using the split middle method
\parencite{morley_graphical_1991}\footnote{This robust method
results in two lines where there are an odd number of points in
a phase. Poor alignment between these lines indicates a poor fit
\parencite{morley_graphical_1991}.}. We expected to see changes in
...
median, \protect\strokeB{} split middle linear trend (see \cref{sec:va}).}
\label{fig:va-n}
\end{figure}
\clearpage
...
an upward trend and the tred was relatively flat for participants 12
...
12, 15 and 18). Contrary to the hypothesised increase in PA, participant
8 showed a significant decrease (${p}$ = .05). Decreases in PA for
participants 16 and 19 were non-significant, as was the meta-analysis
across all participants for PA (${p}$ = .167).  Although there was
decrease in NA for 9 participants (5, 6, 9, 10, 11, 12, 15, 16 and 18),
none of the differences were significant (all ${P}$s < .05). Increases
in NA for participants 7, 8 and 19 were also non-significant, as was the
...
This study predicted that ABM training would reduce attentional
bias, and this would be accompanied by reductions in worry, trait
rumination, symptoms of anxiety and depression, and improvements to
mood variables. Contrary to our hypotheses, we found no reduction
in attentional bias towards towards negative words, no significant
differences between pre and post worry, rumination, symptoms of anxiety
and depression. We also found no overall changes to mood variables in
the hypothesised directions. Also unsuported were our hypotheses that
ABM training would reduce attentional bias towards words associated with
an ideographic current concern, and rumination towards this concern.

Signpost disengagement hypothesis failure here?

\subsection{study RTC/RCL}
...
These results failed to replicate previous findings
\parencite{yang_attention_2015} that ABM training can reduce attentional
bias towards sad words, and that this change is associated with
concurrent reductions in trait rumination and depressive symptoms. One
explanation for these null findings is that the study was underpowered
to detect post-ABM training outcomes. \textcite{yang_attention_2015}
found effect sizes of ${d}$=.49 for RRS, ${d}$=1.50 for BDI-II and
${d}$=.88 STAI-T. In the current study, with 12 participants and
$\alpha$ set at .05, two-tailed calculations of power (1-$\beta$) gave
\Sexpr{rrs_pwr} for RRS, \Sexpr{bdi_pwr} for BDI-II and \Sexpr{stai_pwr}
for STAI-T. \textcite{yang_attention_2015} did not measure worry,
but in a similar paradigm, \textcite{enock_attention_2014} reported
${d}$=.51 for PSWQ which would give the current study power (1-$\beta$)
of \Sexpr{pswq_pwr}. Although the study used different instruments to
measure symptoms of depression (PHQ-9 vs. BDI-II) and anxiety (GAD-7
vs. STAI-T), it was not underpowered in this regard yet failed to detect
effects. However, participants in this study had low initial symptoms
of depression (in contrast with \textcite{yang_attention_2015}), and
anxiety (see \cref{tab:prepost}). These calculations suggest additional
power may have been required to detect changes in rumination (RRS) and
worry (PSWQ), both of which showed moderate but non-significant effects
(see \cref{tab:prepost}).


sad vs. negative stimuli

Consistent with different effects in different populations? See Williams
et al., but what about specifically rumination in non-depressed.

it also sought to test the hypothesis that rumination occurs as
a result of an impaired ability to disengage from negative self-referent
information \parencite{koster_understanding_2011}.

Was the negative information self-referent enough? Possibly not in the
case of N-words, but it should have been in the case of I-words.

GRS validity. Changing ``goal'' to ``problem'' may invalidate further.

Depression items validity.

Low on rumination so exposure duration not suited.
Were most Ps high worry but the dot-probe exposure configured for high rumination (1250ms)?
"At screening, the 15 participants who began the study had a mean PHQ-8
score of 4.9 (SD=1.7), a mean RRS score of 47.3 (SD=10.5), and a mean
PSWQ score of 63.7 (SD=8.3)."
Did it work for some but not others? e.g. P11 sig. on N-words, large GRS PND (not convinced, others had higher RRS)

Visual analysis vs. inferential.

\subsection{wider RTC/RCL}

Diversity of reasons for biases.

\subsubsection{Measurement: (Emotional) Stroop and dot-probe}
stimuli

sad vs. negative stimuli in this study
depression themes
schemata
Emotional Stroop. Single vs. multiple concurrent stimuli
\parencite{williams_cognitive_1997} in depression? But can only measure
not modify.
...
"Patients with social phobia and a concurrent depressive disorder behaved
like non-patients, indicating that concurrent depression abolishes the
attentional bias." \parencite{musa_selective_2003}

"Overall, the results emphasise the importance of assessing comorbidity
when investigating attentional biases." \parencite{musa_selective_2003}

\subsubsection{Goal progress and current concerns}

"Previous Stroop studies (see Williams, 1988, 1997 for a review)
are consistent with the suggestion that attentional biases in anxiety
disorders are largely restricted to disorder specific concerns. However,
dot-probe studies with anxious patients have so far failed to find
convincing evidence for content specificity. In particular, MacLeod
et al. (1886) and Mogg et al. (1992) both found that the bias towards
detection of physical-threat words that occurs found in GAD patients is
unrelated to the extent to which the patients' concerns are predominately
social or physical." \parencite{musa_selective_2003}

"Nunn et al. (1997) suggested that their finding of a depression-congruent
bias on the Stroop task depended on self-referent elaborative processing,
so this finding may not reflect a bias in early attentional processes"
\parencite{mogg_biases_2000}
...
"words that were judged to be highly related to likely concerns
or relevant threats caused more interference than those
which were not, irrespective of their positive or negative
valence." \parencite{mathews_emotionality_1993}

Does this question the use of general vs. ideographic stimuli in the
dot-probe? Look back at the (bi-polar ambiguity of) the valence ratings in
this study?!!!
...
to ruminate.} \parencite{schultheiss_role_2008}

Could individual's current concern(s) indicate the type of bias
...
But Dose response? trend reversal in NA for p8 but maybe
not enough sessions (but loads and people won't keep them up!) With a minimum of 8 
note that p11 was the only sig. and high PND and also consistently did the most (24)
(see \cref{tab:stim}).
Longevity of effects treatment? Would ABA revert? Problem as
people unlikely to keep up training. "the training-induced attentional
avoidance of negative information bias remained in evidence 24 hours after
completion of the final training session and appears to have influenced
anxiety reactions to a stressful event encountered 48 hours after the
completion of this training." \parencite{see_reduction_2009}

"A central feature of MBCT is teaching skills in attentional deployment
through the practice of mindfulness and essentially training individuals
to disengage from negative information and reduce rumination.  [...] The
incorporation of measures of attention and rumination into future trials
of MBCT and assessing their role in determining MBCT outcome will be
a useful step not only in further understanding this form of therapy
but also in helping to reveal the key information processes involved in
depressive relapse." \parencite{donaldson_rumination_2007}

\subsubsection{Treatment efficacy}
...
\parencite{cristea_efficacy_2015}. \parencite{macleod_attentional_2015}
question this finding, arguing that a selection criterion for
meta-anlysis should be that studies demonstrate that ABM reduces
...
\parencite{cuijpers_psychotherapies_2015} conclusion is that research
seeking new psychotherapies for depression is misguided, given the many
existing therapies and the difficulty demonstating relative efficacy
with sufficient statistical power.

In spite of it's potential cost savings of computer delivered CBM it's effectiveness is suspect.

But why are existing therapies relatively ineffective? Because they're
treating the wrong things in different people with the wrong treatments.

Given the larger effect sizes for CBM-I than CBM-A and the likelihood
that depressive rumination is associated with later information processing
biases I RFW
...
However, existing research is limited and equivocal as to whether
...
demonstrate symptom change \parencite{micco_efficacy_2014}.

The presence of a bias doesn't determine the most effective treatment.
...
Maybe measurement of AB will provide support for cognitive therapies
e.g. the interference mechanisms with the largest effects are 'schemata'
so ABM may be only temporary relief? So RFW on measurement rather than
modification? \parencite{williams_cognitive_1997}

\subsubsection{Mechanisms and Causality}

Still unclear and under researched

\subsection{Limitations (300-400)}

Whilst the multiple baseline case-series design lacks a control
condition, it does provide some controls against alternative
explanations of effects including regression to the mean, baseline
assessment effects, extraneous events correlated with phase transition
\parencite{watson_nonconcurrent_1981,watkins_ruminationfocused_2007}.
Furthermore, as this was an exploratory study, limiting Type I error
rates was not a priority.

\begin{itemize}

\item RT dot-probe has poor internal consistency and test-retest reliability
in non-clinical samples \textcite{schmukle_unreliability_2005}
(only measuring chance error variance?). ERP isn't much better than
RT \parencite{kappenman_behavioral_2014}.

\item could have easly matched the stimulus exposure to the RRS (1250ms)/PSWQ(<ms) score!
...
\item correlations between PANAS-SF and depression

\item Did I really demonstrate *disengagement*? Felicity
said the dot-probe doesn't do this. does 'avoidance' ==
'disengagement'? "prolonged processing of self-referent material is
due to impaired attentional disengagement from negative self-referent
information." \parencite{koster_understanding_2011}

\item self-report measures, there are more objective measures? eye-tracking?

\item "Recent laboratory work suggests that biased attentional responding
to negative information causally impacts anxiety reactivity to a
contrived laboratory stressor. However, it remains unknown whether such
attentional bias contributes to real-world anxiety vulnerability. In
the present study, the authors addressed this issue by experimentally
inducing biased attentional response to emotionally negative stimuli,
using a home-based attentional training program and then examining the
influence of this attentional manip- ulation on trait anxiety scores
and on state anxiety responses to a subsequent stressful life event."
\parencite{see_reduction_2009}
...
Need more tasks which are more reliable and precise measures of AB than dot-probe. Need same validation for
for rumination and depression.  \parencite{kappenman_behavioral_2014}.

Inhibitory control \parencite{koster_understanding_2011}.

More precise mechanistic hypotheses e.g. "rumination was related
to attentional switching impairments, specifically in the context of
emotional information" \parencite{koster_rumination_2013}

"The combined cognitive bias hypothesis in depression" \parencite{everaert_combined_2012}
...
(all ${P}$s {\textgreater} .16) (see \cref{tab:nwordcharacteristics}). In
...
\caption{N-word stimulus pairs used in dot-probe and attentional bias training}
\label{tab:nwordlist}
{\tiny

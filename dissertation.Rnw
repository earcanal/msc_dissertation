...
# PSWQ d=1.49 (Amir & Taylor, 2012)
pswq_pwr <- pwr.t.test(d=1.49,n=12,sig.level=0.05,type="paired",alternative="two.sided")
pswq_pwr <- format_power(pswq_pwr$power)
@

Paired samples t-tests were used to compare pre and post ABM training
measures of rumination (RRS), depression (PHQ-9), anxiety (GAD-7)
and worry (PSWQ). \textcite{yang_attention_2015} found significant
post ABM training reductions in rumination (RRS ${d}$=.49), depression
(BDI-II \parencite{beck_manual_1996} ${d}$=1.50) and anxiety (STAI-T
\parencite{spielberger_manual_1983} ${d}$=.88). Given these effect sizes,
12 participants and $\alpha$ set at .05, two-tailed power calculations
give (1-$\beta$) of \Sexpr{rrs_pwr} for RRS, \Sexpr{bdi_pwr} for BDI-II
and \Sexpr{stai_pwr} for STAI-T. In practice, these are likely to be lower
...
Using \textcite{amir_combining_2012} with .7 correlation which
is conservative given test-retest reliability of PSWQ > .9
\parencite{meyer_development_1990} PSWQ power (1-$\beta$) of
\Sexpr{pswq_pwr}.  But ... not dot-probe and combined with CBT

...
\parencite{koster_understanding_2011}. For example, it remains
unclear whether rumination is an automatic or consciously
...
leads to attentional bias \textcite{koster_understanding_2011}.
Although \parencite{morrison_role_2008} found a causal relationship
...
be required to support the disengagement hypothesis. However, there is
evidence in support of this hypothesis.  In a double-blind, randomised
...
tasks, with biases of attention, interpretation and memory most frequently
targetted. The dot-probe task measures attentional bias using differences
in response times to probes which appear at the location of one of a pair
of visual stimuli having differing affective valence, such as negative
and neutral word pairs \parencite{macleod_attentional_1986}. Attentional
bias modification (ABM) manipulates the probe location, to train
towards or away from particular stimuli. A large number of trials
...
shown to reduce depression \parencite{yang_attention_2015}. Critics of
CBM argue that the effect sizes are too small to be clinically relevant
\parencite{cristea_efficacy_2015}. However, advocates argue that this
critique is based on meta-anlyses don't differentiate ABM from other
forms of CBM, focus on a particular psychological disfunction (anxiety),
and analyse studies which demonstrate both attentional bias change
${and}$ corresponding symptom change. It is important to note, that in
demonstrating the mediating role of rumination in reducing depressive
symptoms, \textcite{yang_attention_2015} met both of this latter criteria.

\paragraph{RAT}

GAP

Most studies which examine rumination towards current concerns,
use the emotional Stroop task as a measure of attentional bias
\parencite{williams_cognitive_1988}. By using the dot-probe task, the
current study aims to measure attentional bias towards a current concern
and then measure the effects of modifying the bias using ABM training
in the form of a modified dot-probe. Given the broad conceptualization
of rumination, \textcite{smith_roadmap_2009} recommend using multiple
outcome measures. The difficulties in distinguising
 rumination from worry and the comorbidity of anxiety and depression also
 suggest multiple measures. As such, the current study measures trait and
 state rumination, trait worry, mood variables, anxiety and depression.
...
This is a novel and more direct test of RNT and a potential
intervention to reduce rumination and treat anxiety and depression.

can I argue (using Macleod's meta analysis critique) that I'm getting closer to causality?

\paragraph{WTD}

The current study measures both worry and rumination

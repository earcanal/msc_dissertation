...
The dot-probe task is commonly used to measure attentional bias, based
on differences in response times to pairs of stimuli with differing
affective valence \parencite{macleod_attentional_1986}. Our study
used word-pair stimuli (see \cref{fig:dotprobe}) and was based on
\textcite{yang_attention_2015}.
